...
#         (
#             f"相对品类 "
#             f"{deviation_text(result.get('cvr_deviation'))}"
#             if result.get(
#                 "cvr_deviation"
#             )
#             is not None
...
#         required_data = effective_action.get(
#             "required_data",
#             []
#         )
...
# action_type = effective_action.get(
#     "action_type"
# )

# target_stage = effective_action.get(
#     "target_stage"
# )


# action_top_cols = st.columns(
#     4
# )
...
# st.caption(
#     "基于真实电商漏斗、品类基准、价格位置与历史反馈进行 SKU 转化诊断"
# )
...
#         当前 SKU Purchase CVR 为
#         <b>{pct(result.get("purchase_cvr"))}</b>，
#         Category Benchmark 为
#         <b>{pct(result.get("category_cvr"))}</b>，
#         相对偏离
#         <b>{deviation_text(result.get("cvr_deviation"))}</b>。

#         <br><br>

#         当前诊断状态为：
#         <b>{STATUS_MAP.get(status, status)}</b>。

#         <br><br>

#         主要弱环节定位为：
#         <b>{STAGE_MAP.get(result.get("weak_stage"), "—")}</b>。
...
#     st.warning(
#         "当前 Product Sessions 未达到最低诊断门槛 20，"
#         "本次仅展示原始数据，不进行深度异常判断。"
#     )
...
#         st.write(
#             f'✓ 主要弱环节：'
#             f'{STAGE_MAP.get(result.get("weak_stage"), "—")}'
#         )

#         st.write(
#             f'✓ 价格状态：'
#             f'{PRICE_MAP.get(result.get("price_status"), "—")}'
#         )
...
#         purchase_change = comparison.get(
#             "purchase_cvr_change"
#         )
...
# st.caption(
#     "基于 TheLook 数据、Category Benchmark、规则引擎和 LangGraph 的 SKU 转化诊断"
# )
...
#                     if product_to_cart_deviation is not None
#                     else "未执行"
#                 )
#             )
...
#                     if cart_to_purchase_deviation is not None
#                     else "未执行"
#                 )
#             )
